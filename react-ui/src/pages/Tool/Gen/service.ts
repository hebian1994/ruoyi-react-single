...
// 生成代码（自定义路径）
export async function genCodeREACT(tableName: string) {
  return request(`/api/code/gen/genCodeREACT/${tableName}`, {
    method: 'GET',
  });
}
...
// 生成代码（压缩包）
export async function batchGenCodeReact(tableName: string) {
  return downLoadZip(`/api/code/gen/batchGenCode?tables=${tableName}`);
}
